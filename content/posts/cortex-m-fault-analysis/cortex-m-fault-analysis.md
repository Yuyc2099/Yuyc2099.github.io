...
发生异常时，Cortex-M 内核在跳转到异常向量前会**自动**将基本栈帧压入当前活动栈（MSP 或 PSP）：

![Cortex-M 基本异常栈帧与双字对齐](images/cortex-m-basic-exception-stack-frame.png)

Cortex-M 使用满递减栈。按寄存器的概念压栈顺序，硬件先压入 xPSR，随后依次压入 PC、LR、R12、R3、R2、R1，最后压入 R0。由于 SP 向低地址移动，先压入的 xPSR 最终位于高地址，最后压入的 R0 位于低地址；异常入口完成后，SP 指向 R0。下面将寄存器、相对 SP 的偏移和用途合并在栈帧中，并统一把低地址画在上方、高地址画在下方：

```text
低地址
异常入口后的 SP -->   +--------------------------+
                     | +0x00  R0                |  参数/返回值寄存器；最后压栈
                     +--------------------------+
                     | +0x04  R1                |  参数寄存器
                     +--------------------------+
                     | +0x08  R2                |  参数寄存器
                     ...
                     | +0x18  PC                |  出错时的程序计数器
                     +--------------------------+
                     | +0x1C  xPSR              |  程序状态寄存器（IPSR/EPSR/APSR）；最先压栈
                     +--------------------------+
                     | +0x20  Padding           |  可选对齐填充字；xPSR[9] = 1 时存在
异常发生前的 SP -->   +--------------------------+
                     | 异常发生前已使用的栈空间   |
                     +--------------------------+
高地址
